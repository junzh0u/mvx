...
fn verify_file_moved(src_path: &PathBuf, dest_path: &PathBuf, expected_content: &str) {
    ...
    assert!(!src_path.exists(), "Source file still exists at {}", src_path.display());

    // Check that the destination file exists and has the correct content
    assert!(dest_path.exists(), "Destination file does not exist at {}", dest_path.display());
    let moved_content = fs::read_to_string(dest_path).unwrap();
    assert_eq!(moved_content, expected_content, "File content doesn't match after move");
}
...
fn test_move_file_to_directory() {
    ...
    assert!(result.is_ok(), "Failed to move file to directory: {:?}", result.err());
...
fn verify_pre_existing_files(files: &Vec<PathBuf>, contents: &Vec<String>) {
    for (i, file_path) in files.iter().enumerate() {
        assert!(file_path.exists(), "Pre-existing file {} no longer exists", file_path.display());

        // Verify content is unchanged
        let content = fs::read_to_string(file_path).unwrap().trim().to_string();
        assert_eq!(content, contents[i], "Content changed for pre-existing file {}", file_path.display());
    }
}
...
fn test_merge_directories() {
    ...
    for src_file in &src_files {
        ...
        src_file_contents.push((src_file.strip_prefix(&src_path).unwrap().to_path_buf(), content));
    }

    // Run the mvx command to merge directories
    let result = run_mvx(&src_path, dest_path.to_str().unwrap());
    assert!(result.is_ok(), "Failed to merge directories: {:?}", result.err());
    ...
    for (rel_path, content) in &src_file_contents {
        ...
        assert!(dest_file.exists(), "File {} not found in destination", dest_file.display());

        // Verify content matches what was in the source
        let moved_content = fs::read_to_string(&dest_file).unwrap();
        assert_eq!(&moved_content, content, "Content doesn't match for moved file {}", dest_file.display());
    }

    // Check that the subdirectory was moved
    let moved_subdir_path = dest_path.join("subdir");
    assert!(moved_subdir_path.exists() && moved_subdir_path.is_dir(),
    ...
    assert!(dest_subdir_path.exists() && dest_subdir_path.is_dir(),
...
fn test_overwrite_existing_file() {
    ...
    assert!(dest_path.exists(), "Destination file was not created properly for test setup");
    let original_dest_content = fs::read_to_string(&dest_path).unwrap();
    assert!(original_dest_content.contains(dest_content),
    ...
    let result = run_mvx(&src_path, dest_path.to_str().unwrap());
    assert!(result.is_ok(), "Failed to move and overwrite file: {:?}", result.err());
...
fn test_overwrite_file_in_directory_merge() {
    ...
    assert!(src_conflict_path.exists(), "Source conflict file doesn't exist");
    assert!(dest_conflict_path.exists(), "Destination conflict file doesn't exist");
    ...
    assert_eq!(new_dest_content, src_content,
              "Destination file was not properly overwritten during directory merge");

    // Check that the source directory no longer exists
    assert!(!src_path.exists(), "Source directory still exists after merge");
}
...
fn test_move_to_nonexistent_directory_path() {
    ...
    assert!(!nonexistent_dir_path.exists(), "Test directory should not exist before the test");

    // Run the mvx command
    let result = run_mvx(&src_path, &dest_path_str);
    assert!(result.is_ok(), "Failed to move file to nonexistent directory path: {:?}", result.err());

    // Check that the destination directory was created
    assert!(nonexistent_dir_path.exists(), "Destination directory was not created");
    assert!(nonexistent_dir_path.is_dir(), "Destination path is not a directory");
...
fn test_move_to_nonexistent_file_path() {
    ...
    assert!(!nonexistent_parent_dir.exists(), "Test directory should not exist before the test");

    // Run the mvx command
    let result = run_mvx(&src_path, dest_file_path.to_str().unwrap());
    assert!(result.is_ok(), "Failed to move file to nonexistent file path: {:?}", result.err());

    // Check that the parent directories were created
    assert!(nonexistent_parent_dir.exists(), "Parent directories were not created");
    assert!(nonexistent_parent_dir.is_dir(), "Parent path is not a directory");
...
fn test_error_cant_create_parent_directory() {
    ...
    assert!(blocking_file_path.exists(), "Blocking file was not created properly");
    assert!(blocking_file_path.is_file(), "Blocking path should be a file, not a directory");
    ...
    assert!(result.is_err(), "Expected an error when parent directory can't be created, but got success");
    ...
    assert!(err_msg.contains("Not a directory") ||
            ...
            "Error message doesn't mention directory creation issue: {}", err_msg);

    // Verify the source file still exists (wasn't deleted)
    assert!(src_path.exists(), "Source file was deleted despite the operation failing");

    // Verify the destination file wasn't created
    assert!(!dest_path.exists(), "Destination file was created despite error");

    // Verify the blocking file wasn't modified
    assert!(blocking_file_path.is_file(), "Blocking file was converted to a directory");
}
...
fn test_error_nonexistent_source() {
    ...
    // Run the mvx command
    let result = run_mvx(&src_path, dest_path.to_str().unwrap());
    assert!(result.is_err(), "Expected an error for non-existent source, but got success");

    // Check that the error message contains the expected text
    let err_msg = result.err().unwrap().to_string();
    assert!(err_msg.contains("does not exist"),
            "Error message doesn't mention that source doesn't exist: {}", err_msg);
